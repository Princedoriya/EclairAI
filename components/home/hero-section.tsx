...
export default function HeroSection({ hasReachedLimit }: HeroSectionProps) {
    const router = useRouter();

    const handleTryEclairClick = useCallback(() => {
        if (hasReachedLimit) {
        ...
        <MotionSection
         ...
         className='relative mx-auto flex flex-col z-0 items-center justify-center py-16 sm:py-20 lg:pb-28 transition-all animate-in lg:px-12 max-w-7xl '>
                ...
                <MotionDiv variants={itemVariants} whileHover={buttonVariants}>
                    <Button onClick={handleTryEclairClick} variant={'link'} className='text-white mt-6 text-base sm:text-lg lg:text-xl rounded-full px-8 sm:px-10 lg:px-12 py-6 sm:py-6 lg:py-8 lg:mt-16 bg-linear-to-r from-slate-900 to-rose-500 hover:from-rose-500 hover:to-slate-900 hover:no-underline font-bold shadow-lg transition-all duration-300'>
                        <span>Try EclairAI</span>
