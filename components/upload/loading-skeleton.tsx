...
export default function LoadingSkeleton() {
  return (
    <Card
      className="relative px-2 h-[700px] w-[600px] max-w-lg mx-auto overflow-hidden bg-linear-to-br from-background via-background/95 to-rose-500/5 backdrop-blur-lg shadow-2xl rounded-3xl border
               border-rose-500/10"
    >
      {/* Loading Progress Bar */}
      <div
        className="absolute top-0 left-0 right-0 z-20 bg-background/80 backdrop-blur-xs pt-4 pb-2 border-b
                border-rose-500/10"
      >
        <div className="px-4 flex gap-1.5">
          {[1, 2, 3].map((_, index) => (
            ...
            >
              <div
                className={cn(
                  "h-full bg-linear-to-r from-gray-50 to-rose-600 animate-pulse",
                  index === 0 ? "w-full" : "w-0"
                )}
              />
            </div>
        ...
        <div className="px-6">
          ...
          <div className="space-y-4">
            ...
            {[1, 2, 3].map((_, index) => (
              <div
                key={`numbered-${index}`}
                className="group relative bg-linear-to-br from-gray-500/[0.08] p-4 rounded-2xl border to-gray-600/[0.03] border-gray-500/10"
              >
      ...
      <div
        className="absolute bottom-0 left-0 right-0 p-4 bg-background/80 backdrop-blur-xs border-t
                border-rose-500/10"
      >
        <div className="flex justify-between items-center">
          <Skeleton className="rounded-full w-12 h-12 bg-linear-to-br from-rose-500/50to-rose-600/50" />
